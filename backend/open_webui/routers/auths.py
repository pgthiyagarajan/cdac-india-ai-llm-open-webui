...
_CAPTCHA_CHARS = 'ACDEFGHJKLMNPQRTUVWXY234679'


def _generate_captcha_svg(text: str) -> str:
    """Render CAPTCHA text as a distorted SVG (no external dependencies)."""
    width, height = 170, 44
    chars_svg = []
    for i, ch in enumerate(text):
        x = 16 + i * 24 + random.randint(-3, 3)
        y = 24 + random.randint(-5, 5)
        rotate = random.randint(-18, 18)
        fill = random.choice(['#e2e8f0', '#fbbf24', '#a5b4fc', '#6ee7b7', '#fca5a5', '#93c5fd'])
        size = random.randint(17, 22)
        chars_svg.append(
            f'<text x="{x}" y="{y}" font-size="{size}" fill="{fill}" font-family="monospace" '
    ...
    for _ in range(3):
        ...
        noise_lines.append(
            ...
            f'stroke="#64748b" stroke-width="1" opacity="0.4"/>'
        )
    body = '\n'.join(noise_lines + chars_svg)
    return (
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" '
        f'style="background:#1e293b;border-radius:8px;display:block">{body}</svg>'
    )
...
async def get_captcha(request: Request):
    """Generate a server-side CAPTCHA and return its SVG + a one-time token."""
    text = ''.join(random.choices(_CAPTCHA_CHARS, k=6))
    token = secrets.token_urlsafe(20)
    if request.app.state.redis:
        await request.app.state.redis.setex(
            f'{REDIS_KEY_PREFIX}:captcha:{token}', _CAPTCHA_TTL, text
        )
    import base64 as _b64
    svg = _generate_captcha_svg(text)
    svg_b64 = _b64.b64encode(svg.encode()).decode()
    return {'token': token, 'image': f'data:image/svg+xml;base64,{svg_b64}'}
...
async def update_password(
    ...
    if session_user:
        ...
        if user:
            ...
            # CERT-In: revoke all existing sessions on password change
            if updated and request.app.state.redis:
                await request.app.state.redis.set(
                    f'{REDIS_KEY_PREFIX}:auth:user:{user.id}:revoked_at',
                    str(int(time.time())),
                )
            return updated
        else:
...
async def signin(
    ...
    if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        # CERT-In: server-side CAPTCHA validation
        if not await _validate_captcha(request.app.state.redis, form_data.captcha_token, form_data.captcha_answer):
            raise HTTPException(
    ...
    if user:
        # CERT-In: terminate all prior sessions on new login (concurrent session control)
        if request.app.state.redis:
            await request.app.state.redis.set(
                f'{REDIS_KEY_PREFIX}:auth:user:{user.id}:revoked_at',
                str(int(time.time())),
...
async def signout(request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
    ...
    if oauth_session_id:
        ...
        # Parichay has its own revocation endpoint
        if session and session.provider == 'parichay':
            from open_webui.config import PARICHAY_API_URL
            access_token = session.token.get('access_token')
            if access_token:
                try:
                    revoke_url = f"{PARICHAY_API_URL.value.rstrip('/')}/pnv1/salt/api/oauth2/revoke"
                    async with ClientSession(trust_env=True) as client_session:
                        async with client_session.get(
                            ...
                            log.info(f'Parichay token revoke: {r.status}')
                except Exception as e:
                    log.error(f'Parichay token revoke failed: {e}')
            await OAuthSessions.delete_session_by_id(session.id, db=db)
