...
class OAuthClientManager:
    ...
    async def handle_callback(self, request, client_id: str, user_id: str, response):
        ...
        try:
            ...
            log.warning(
                'OAuth callback error for user_id=%s client_id=%s',
                user_id,
                client_id,
            )
...
_PARICHAY_PKCE_TTL = 600  # seconds — enough for user to complete NIC login
...
class OAuthManager:
    ...
    async def _perform_token_refresh(self, session) -> dict:
        ...
        # Parichay uses a non-standard refresh flow — Authorization header, JSON body
        if provider == 'parichay':
            refresh_token = token_data.get('refresh_token')
            if not refresh_token:
                log.warning(f'No refresh token available for Parichay session {session.id}')
                return None
            try:
                token_url = f"{PARICHAY_API_URL.value.rstrip('/')}/pnv1/salt/api/oauth2/token"
                async with aiohttp.ClientSession(trust_env=True) as session_http:
                    async with session_http.post(
                        token_url,
                        ...
                        if r.status == 200:
                            new_token = await r.json()
                            if 'refresh_token' not in new_token:
                                new_token['refresh_token'] = refresh_token
                            _normalize_token_expiry(new_token)
                            return new_token
                        error_text = await r.text()
                        log.error(f'Parichay token refresh failed: {r.status} — {error_text}')
                        return None
            except Exception as e:
                log.error(f'Parichay token refresh exception: {e}')
                return None
    ...
    async def handle_login(self, request, provider):
        ...
        # Parichay requires manual PKCE — authlib's standard flow doesn't fit its endpoint
        if provider == 'parichay':
            ...
            if redis:
                ...
                await redis.setex(
                    f'{REDIS_KEY_PREFIX}:pkce:{state}',
                    _PARICHAY_PKCE_TTL,
                    _json.dumps({'state': state, 'code_verifier': code_verifier}),
                )

            redirect_uri = PARICHAY_REDIRECT_URI.value or str(
                request.url_for('oauth_login_callback', provider='parichay')
            )
            params = {
                'response_type': 'code',
                'client_id': PARICHAY_CLIENT_ID.value,
            ...
            authorize_url = f"{PARICHAY_API_URL.value.rstrip('/')}/pnv1/oauth2/authorize"
            return StarletteRedirect(f"{authorize_url}?{urlencode(params)}")

        # Generic OIDC flow for all other providers
    ...
    async def handle_callback(self, request, provider, response, db=None):
        ...
        try:
            ...
            if provider == 'parichay':
                ...
                if redis:
                    import json as _json
                    key = f'{REDIS_KEY_PREFIX}:pkce:{state}'
                    raw = await redis.get(key)
                    if raw:
                        await redis.delete(key)
                        state_data = _json.loads(raw)
                if not state_data or state_data.get('state') != state:
                    log.error(f'Parichay CSRF state mismatch or missing for state={state}')
                    raise HTTPException(400, detail='CSRF check failed')
                ...
                payload = {
                    'client_id': PARICHAY_CLIENT_ID.value,
                    'client_secret': PARICHAY_CLIENT_SECRET.value,
                ...
                async with aiohttp.ClientSession(trust_env=True) as http:
                    async with http.post(
                        ...
                        if r.status != 200:
                            log.error(f'Parichay token exchange failed: {r.status} — {await r.text()}')
                            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
                        token = await r.json()

                access_token = token.get('access_token')
                userdetails_url = f"{PARICHAY_API_URL.value.rstrip('/')}/pnv1/salt/api/oauth2/userdetails"
                async with aiohttp.ClientSession(trust_env=True) as http:
                    async with http.get(
                        userdetails_url,
                        ...
                        if r.status == 200:
                            ...
                            async with http.get(
                                ...
                                if r2.status != 200:
                                    log.error(f'Parichay userdetails failed: {r2.status}')
                                    raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
                                user_raw = await r2.json()

                sub = user_raw.get('parichayId')
                email = user_raw.get('Email') or user_raw.get('EmailId') or user_raw.get('loginId')
                name = (
                    f"{user_raw.get('FirstName', '')} {user_raw.get('LastName', '')}".strip()
                    or user_raw.get('userName')
                    or email
                )
                if not sub or not email:
                    log.error(f'Parichay profile missing parichayId or email: {user_raw}')
                    raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)

                user_data = UserInfo({'sub': sub, 'email': email, 'name': name, **user_raw})
                id_token_claims = dict(user_data)

            else:
                # --- Generic OIDC / OAuth2 callback ---
                auth_params = {}
                if client:
                ...
                try:
                    ...
                    try:
                        ...
                        log.warning('OAuth callback retry failed for provider %s', provider)
                        raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
                except Exception:
                    log.warning('OAuth callback error for provider %s', provider)
                    raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_CRED)
